...
the runtime processing in the tool itself. The two differ deliberately,
and §7 sets out how.
...
| Brysbaert et al. (2025) ECP supplement | AoA extension | AI-generated (fine-tuned GPT-4o). Adds 4,042 words not in Kuperman; values cluster at round numbers (see §2). |
...
## 2. The ECP supplement caveat

The 2025 Brysbaert ECP file adds 3,994 words not in Kuperman, of which 334 appear
...
## 3. Text processing (validation pipeline)
...
## 4. British–American normalisation
...
defensible plain-language target as well as the empirical optimum. This sweep was
exploratory and lives in the project notes rather than a standalone script, so it
is the one figure here without a reproduction entry in §8.
...
|---|---|---|
| Tokenisation | spaCy | letter runs with internal hyphens/apostrophes kept; hyphenated compounds split and scored per component; internal apostrophes stripped for lookup |
| Lemmatisation | yes (spaCy) | **none** — surface form only |
| POS filtering | content words only | none — every word is looked up |
| Proper nouns | handled by lemmatisation | capitalised word absent from the SUBTLEX top 10k skipped as a name (sentence-initial words exempt; override terms always scored) |
...
The batch scorer (`score_decisions.py`) applies the same flag rule and
per-occurrence counting to spreadsheet rows, but tokenises on plain `[A-Za-z]+`
runs: no apostrophe stripping and no proper-noun exclusion, so capitalised names
whose lowercase form has an AoA rating are scored there but skipped by the tool.
Rows with fewer than 20 content words are not scored.
...
| Full ECP merge into the lookup | `validation/merge_ecp_all.py` |
| Override surface-variant additions | `validation/add_override_variants.py` |
